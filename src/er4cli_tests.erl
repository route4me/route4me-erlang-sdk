...
-define(TST_TOKEN, "11111111111111111111111111111111").

gargs() ->
	#er4args{token = ?TST_TOKEN}.
...
%%%----------------------------------------------------------------------
test_get_route_q() ->
	test_print0(<<"Gets a status update on the pending optimization problems:">>, er4cli:get_route_q()).

test_get_route() ->
	test_print0(<<"Gets a Route info by ID:">>, er4cli:get_route_by_id("AC16E7D338B551013FF34266FE81A5EE")).

test_delete_route_non_existing() ->
	test_print0(<<"Deletes a Route by non-existing ID:">>, er4cli:delete_route("AC16E7D338B551013FF34266FE81A5EEXXX")).
...
test_get_route_manifest() ->
	...
	test_print0(<<"Gets Route manifest:">>, er4cli:get_route(Params)).
...
test_set_gps() ->
	...
	test_print0(<<"Set GPS:">>, er4cli:set_gps(Params)).
...
test_track_device_last_location_history() ->
	...
	test_print0(<<"Gets Route manifest for GPS location history:">>, er4cli:get_route([{route_id, RouteID},{device_tracking_history, 1}])).
...
test_single_driver_round_trip() ->
	...
	test_print0(<<"Single Driver Round Trip:">>, er4cli:run_optimization(test_addrs_single_driver_round_trip(), Params)).
...
test_single_driver_route_10_stops() ->
	...
	test_print0(<<"Single Driver Route 10 Stops:">>, er4cli:run_optimization(test_addrs_single_driver_route_10_stops(), Params)).
...
test_multiple_depot_multiple_driver_time_window() ->
	...
	test_print0(<<"Multiple Depot, Multiple Driver, Time window:">>, er4cli:run_optimization(test_addrs_TSP_TW(), Params)).
...
test_multiple_depot_multiple_driver_with_24_stops_time_window() ->
	...
	test_print0(<<"Multiple Depot, Multiple Driver (with 24 stops time window):">>, er4cli:run_optimization(test_addrs_TSP_TW(), Params)).
...
test_optimize_and_reoptimize() ->
	...
	case er4cli:run_optimization(test_addrs_single_driver_round_trip(), Params) of
		{ok, {struct, Props}} ->
			OPId = proplists:get_value("optimization_problem_id", Props, "undefined"),
			io:format("Parsed optimization problem ID: ~p~n", [OPId]),
			test_print0(<<"Reoptimize:">>, er4cli:reoptimize(OPId));
		{ok, Resp} ->
			io:format("Reoptimize: Ok. Json object is:~n~p~n", [Resp]);
		{error, Why, Details} ->
			io:format("error (in reoptimize): ~p ~p: ~p: ~p~n", [?MODULE, ?LINE, Why, Details])
